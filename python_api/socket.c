...
void send_msg ( int sockfd, char * outMsgP, int len)
{   
    int numbytes;
    if ((numbytes = sendto(sockfd,(unsigned char*)outMsgP , len, 0,(struct sockaddr *)&their_addr, sizeof(struct sockaddr))) == -1)
    {
        perror("Send to ");
        exit(0);
    }
